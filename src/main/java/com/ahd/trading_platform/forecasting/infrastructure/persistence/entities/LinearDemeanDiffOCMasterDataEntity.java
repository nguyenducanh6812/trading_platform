...
/**
 * Entity for storing LINEAR market Demean_Diff_OC master data.
 * Supports all symbols in the LINEAR market.
 */
@Entity
@Table(name = "linear_demean_diff_oc_master_data",
       schema = "trading_platform",
       uniqueConstraints = @UniqueConstraint(name = "uk_linear_demean_symbol_timestamp", columnNames = {"symbol", "timestamp"}),
       indexes = {
           @Index(name = "idx_linear_demean_symbol", columnList = "symbol"),
           @Index(name = "idx_linear_demean_timestamp", columnList = "timestamp"),
           @Index(name = "idx_linear_demean_symbol_timestamp", columnList = "symbol, timestamp")
       })
...
@AllArgsConstructor
@Builder
public class LinearDemeanDiffOCMasterDataEntity {
