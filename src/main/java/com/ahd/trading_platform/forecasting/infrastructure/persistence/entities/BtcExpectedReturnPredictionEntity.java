...
/**
 * JPA entity for BTC expected return predictions.
 * Stores ARIMA forecast results with model version tracking.
 */
@Entity
@Table(name = "btc_expected_return_prediction", 
       indexes = {
           @Index(name = "idx_btc_prediction_forecast_date", columnList = "forecast_date"),
           @Index(name = "idx_btc_prediction_model_version", columnList = "model_version"),
           @Index(name = "idx_btc_prediction_execution_id", columnList = "execution_id"),
           @Index(name = "idx_btc_prediction_created", columnList = "created_at")
       },
       uniqueConstraints = {
           @UniqueConstraint(name = "uk_btc_prediction_date_model", columnNames = {"forecast_date", "model_version"})
       })
@Data
@NoArgsConstructor
@AllArgsConstructor
public class BtcExpectedReturnPredictionEntity {
