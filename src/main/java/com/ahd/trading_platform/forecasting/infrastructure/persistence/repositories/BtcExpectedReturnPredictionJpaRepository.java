...
/**
 * JPA repository for BTC expected return predictions
 */
@Repository
public interface BtcExpectedReturnPredictionJpaRepository extends JpaRepository<BtcExpectedReturnPredictionEntity, Long> {
    
    /**
     * Find predictions within a date range
     */
    @Query("SELECT p FROM BtcExpectedReturnPredictionEntity p " +
           "WHERE p.forecastDate >= :startDate AND p.forecastDate <= :endDate " +
           "ORDER BY p.forecastDate DESC")
    List<BtcExpectedReturnPredictionEntity> findByForecastDateBetween(
    ...
    @Query("SELECT p FROM BtcExpectedReturnPredictionEntity p " +
           ...
           "ORDER BY p.forecastDate DESC")
    List<BtcExpectedReturnPredictionEntity> findByModelVersion(@Param("modelVersion") String modelVersion);
    
    /**
     * Find prediction for specific date and model version
     */
    Optional<BtcExpectedReturnPredictionEntity> findByForecastDateAndModelVersion(
    ...
    @Query("SELECT p FROM BtcExpectedReturnPredictionEntity p " +
           ...
           "ORDER BY p.forecastDate DESC " +
           "LIMIT 1")
    Optional<BtcExpectedReturnPredictionEntity> findLatestByModelVersion(@Param("modelVersion") String modelVersion);
    ...
    @Query("SELECT p FROM BtcExpectedReturnPredictionEntity p " +
           ...
           "ORDER BY p.forecastDate DESC")
    List<BtcExpectedReturnPredictionEntity> findSuccessfulPredictions();
     ...
     */
    List<BtcExpectedReturnPredictionEntity> findByExecutionId(String executionId);
