...
public class ExecuteARIMAForecastTaskWorker implements ExternalTaskHandler {
    ...
    private OrchestrationInput extractOrchestrationInput(ExternalTask externalTask) throws InvalidProcessVariablesException {
        try {
            // Extract instrument codes as strings (no conversion needed)
            List<String> instrumentCodes = extractInstrumentCodes(externalTask);
            ...
            // Default values - determine mode based on context
            if (isCurrentDate == null) {
                // Smart default: if startDate and endDate are provided, assume backtesting mode
                // Otherwise, default to current date mode
                boolean hasDateRange = (startDate != null && !startDate.trim().isEmpty()) &&
                                     (endDate != null && !endDate.trim().isEmpty());
                isCurrentDate = !hasDateRange; // false for backtesting, true for current mode

                log.debug("Auto-detected mode based on parameters: isCurrentDate={} (startDate={}, endDate={})",
    ...
    private List<String> extractInstrumentCodes(ExternalTask externalTask) throws InvalidProcessVariablesException {
        ...
        try {
            ...
            if (instrumentCodesVar instanceof List) {
                return (List<String>) instrumentCodesVar;
            } else if (instrumentCodesVar instanceof String) {
                return parseJsonStringArray((String) instrumentCodesVar);
            } else {
                throw new InvalidProcessVariablesException(
    ...
    private BatchForecastOrchestrationResponse executeBatchForecasts(OrchestrationInput input) {
        ...
        // Instrument codes are already strings - no conversion needed
        return executeARIMAForecastUseCase.executeBatch(
            input.instrumentCodes(),
            input.startDate(),
            input.endDate(),
            input.isCurrentDate(),
